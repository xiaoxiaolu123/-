<!--  -->
<template>
  <div class="dashboard">
    <div class="header-box">
      <div class="logo-box">
        <a data-v-46b0c60a="" href="javascript:void(0)">
          <img
            src="data:image/png;base64,"
            width="112" height="30" />
        </a>
      </div>
      <div class="page-name">{{ pageName }}</div>
      <div class="user-info">
        <el-dropdown @command="handleCommand">
          <span class="el-dropdown-link">
            {{ user.name }}
            <i class="el-icon-arrow-down el-icon--right"></i>
          </span>
          <el-dropdown-menu slot="dropdown">
            <el-dropdown-item command="change_pass">修改密码</el-dropdown-item>
            <el-dropdown-item command="logout">退出</el-dropdown-item>
          </el-dropdown-menu>
        </el-dropdown>
      </div>
    </div>

    <div class="page-body">
      <div class="page-body-left-menus">
        <el-row class="tac">
          <el-col :span="12">
            <el-menu :default-active="pageName" class="el-menu-vertical-demo" @select="select" :unique-opened="true">
              <el-menu-item index="主页">
                <i class="iconfont icon-icon-study-n"></i>
                <span slot="title">主页</span>
              </el-menu-item>

              <!-- <el-submenu index="2">
                <template slot="title">
                  <i class="iconfont icon-icon-decorate"></i>
                  <span>装修</span>
                </template>
                <el-menu-item index="电脑端">电脑端</el-menu-item>
                <el-menu-item index="移动端">移动端</el-menu-item>
              </el-submenu> -->

              <el-submenu index="3">
                <template slot="title">
                  <i class="iconfont icon-icon-lesson"></i>
                  <span>课程</span>
                </template>
                <el-menu-item index="录播课">录播课</el-menu-item>
              </el-submenu>

              <el-submenu index="4">
                <template slot="title">
                  <i class="iconfont icon-icon-me-n"></i>
                  <span>学员</span>
                </template>
                <el-menu-item index="学员列表">学员列表</el-menu-item>
              </el-submenu>

              <el-submenu index="5">
                <template slot="title">
                  <i class="iconfont icon-icon-money-n"></i>
                  <span>财务</span>
                </template>
                <el-menu-item index="全部订单">全部订单</el-menu-item>
                <el-menu-item index="余额提现">余额提现</el-menu-item>
              </el-submenu>

              <el-submenu index="6">
                <template slot="title">
                  <i class="iconfont icon-icon-operate"></i>
                  <span>运营</span>
                </template>
                <el-menu-item index="VIP会员">VIP会员</el-menu-item>
                <el-menu-item index="优惠码">优惠码</el-menu-item>
                <el-menu-item index="公众号">公众号</el-menu-item>
              </el-submenu>

              <el-submenu index="7">
                <template slot="title">
                  <i class="iconfont icon-icon-setting-n"></i>
                  <span>系统</span>
                </template>
                <el-menu-item index="管理人员">管理人员</el-menu-item>
                <!-- <el-menu-item index="系统配置">系统配置</el-menu-item>
                <el-menu-item index="功能模块">功能模块</el-menu-item> -->
              </el-submenu>
            </el-menu>
          </el-col>
        </el-row>
      </div>

      <div class="page-main-body-box">
        <router-view></router-view>
      </div>
    </div>
  </div>
</template>

<script>

//这里可以导入其他文件（比如：组件，工具js，第三方插件js，json文件，图片文件等等）
//例如：import 《组件名称》 from '《组件路径》';
import { mapState } from "vuex";
export default {
  //import引入的组件需要注入到对象中才能使用
  components: {},
  data() {
    //这里存放数据
    return {
      pageName: "主页",
    };
  },
  //监听属性 类似于data概念
  computed: {
    ...mapState({
      user: (state) => {
        return state.login.user || {};
      },
    }),
  },
  //监控data中的数据变化
  watch: {
    $route: {
      deep: true,
      immediate: true,
      handler: function (newValue, oldValue) {
        this.pageName = newValue.meta.title;
      }
    }
  },
  //方法集合
  methods: {
    handleCommand: function (command) {
      if (command == "change_pass") {
        this.$router.push({
          path: "/dashboard/change-pass",
        });
      }
      if (command == "logout") {
        this.$request.post("logout").then((res) => {
          localStorage.removeItem("admin-token");
        });
        this.$router.push("/");
      }
    },
    select(key) {
      this.pageName = key;
      switch (key) {
        case "主页":
          this.$router.push("/dashboard");
          break;
        case "电脑端":
          this.$router.push("/decoration/pc");
          break;
        case "移动端":
          this.$router.push("/decoration/h5");
          break;
        case "录播课":
          this.$router.push("/course/vod/index");
          break;
        case "学员列表":
          this.$router.push("/member/index");
          break;
        case "全部订单":
          this.$router.push("/order/index");
          break;
        case "余额提现":
          this.$router.push("/withdrawOrders");
          break;
        case "VIP会员":
          this.$router.push("/role");
          break;
        case "优惠码":
          this.$router.push("/promocode");
          break;
        case "公众号":
          this.$router.push("/wechat/messagereply/index");
          break;
        case "管理人员":
          this.$router.push("/system/administrator");
          break;
        case "系统配置":
          this.$router.push("/system/index");
          break;
        case "功能模块":
          this.$router.push("/system/application");
          break;
      }
    },
  },
  //生命周期 - 创建完成（可以访问当前this实例）
  created() {
    this.$store.dispatch("login/getUserInfo");
  },
  //生命周期 - 挂载完成（可以访问DOM元素）
  mounted() { },
  beforeCreate() { }, //生命周期 - 创建之前
  beforeMount() { }, //生命周期 - 挂载之前
  beforeUpdate() { }, //生命周期 - 更新之前
  updated() { }, //生命周期 - 更新之后
  beforeDestroy() { }, //生命周期 - 销毁之前
  destroyed() { }, //生命周期 - 销毁完成
  activated() { }, //如果页面有keep-alive缓存功能，这个函数会触发
};
</script>
<style  lang='less' scoped>
/deep/ .el-menu-item.is-active {
  background-color: #409eff;
  color: #fff;

  i {
    color: white;
  }
}

/deep/ .el-menu-item,
.el-submenu {
  i {
    font-size: 24px;
    margin-right: 15px;
    color: #556;
  }
}

/deep/ .el-col {
  width: 100%;
}

.dashboard {
  .header-box {
    width: 100%;
    height: 56px;
    display: flex;
    align-items: center;
    position: relative;
    padding-left: 0;
    padding-right: 0;
    background-color: #fff;

    .logo-box {
      margin-left: 44px;
      margin-right: 44px;
      padding-top: 25px;

      a {
        img {
          width: 112px;
          aspect-ratio: auto 112 / 30;
          height: 30px;
        }
      }
    }

    .page-name {
      flex: 1;
      padding-left: 30px;
      font-size: 20px;
      font-weight: 600;
      color: #333;
      line-height: 56px;
      text-align: left;
    }

    .user-info {
      width: auto;
      padding-right: 30px;
    }
  }

  .page-body {
    width: 100%;
    height: calc(100vh - 56px);
    display: flex;

    .page-body-left-menus {
      width: 200px;
      height: 100%;
      overflow-x: hidden;
      overflow-y: auto;
      box-sizing: border-box;
      padding-top: 24px;
      background-color: #fff;
    }

    .page-main-body-box {
      flex: 1;
      height: 100%;
      box-sizing: border-box;
      padding: 20px 5px 20px 20px;
      overflow-y: auto;
      background-color: #f1f2f9;
    }
  }
}
</style>